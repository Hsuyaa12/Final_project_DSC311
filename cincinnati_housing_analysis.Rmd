...
```{r}
#Investigating: What is the relationship between industrial land use (Indus) and housing characteristics such as housing prices, room sizes, and property tax rates?
...
-----------------------------------------------------
```{r}
p1<-ggplot(data = results, aes(x = as.factor(NumVar), y = MSE))+
  geom_point(color="black", size=2) +ylab("MSE") + xlab("No of variables")

p2<-ggplot(data = results, aes(x = as.factor(NumVar), y = AdjR2)) +
