...
CrimeRate - Per capita crime rate by city <br>
Indus - Proportion of non-retail business acres per city.<br>
River – River front or not (1-bounds river; 0-otherwise) <br>
AvgRoom - Average number of rooms per dwelling per city <br>
Age – Average age of a house by city <br>
Tax - full-value property-tax rate per $10,000 <br>
PTRatio - pupil-teacher ratio by city <br>
LStat - % lower status of the population by city <br>
MedPrice - Median price of owner-occupied homes in $1000's <br>
...
# Findings:
All the really low and really high prices outliers come from non-river homes. Riverfront homes all sit in the normal price range. So we can't drop river front houses although their portion in our dataset is not significant. Riverfront does not seem to have much of an impact on any of the other variables.  
...
Interpretation:
The correlation between PTRatio and Crimerate is 0.274, which suggests that as the ratio of students to teachers increases, so does the crimerate. This intuitively makes sense, because less affluent cities will have less teachers for students, and areas such as this are typically more prone to crime. 
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = m$fitted.values, residual = m$residuals)

# Create a residual plot using ggplot2
ggplot(data = temp_df, mapping = aes(x = fitted, y = residual)) +
  geom_point() +
  xlab("Fitted Values") +
  ylab("Residuals") +
...
# Create Q-Q plot
ggplot(temp_df, aes(sample = residual)) +
  stat_qq() +                                  # Generates Q-Q plot
  stat_qq_line(color = "black", linetype = "dashed") +  # Adds reference line
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = m_log$fitted.values, residual = m_log$residuals)
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = m1$fitted.values, residual = m1$residuals)
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = m2$fitted.values, residual = m2$residuals)
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = m3$fitted.values, residual = m3$residuals)
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = full_model1$fitted.values, residual = full_model1$residuals)
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = full_model2$fitted.values, residual = full_model2$residuals)
...
results %>%
  mutate_at(vars(Rsq: MSE), ~ sprintf("%.2f", .)) %>%
  kable(
    format = "html",      
    caption = "Best‐Subset Selection Results",
    align = c("c", rep("c", ncol(results) - 1)),
    booktabs = TRUE
  ) %>%
  kable_styling(
    full_width = FALSE,
  ...
  column_spec(
    1,                            # first column
    bold      = TRUE,             # still bold
    width     = "4em",            # give it some fixed width
...
# ===== Residuals + QQ Plots =====
par(mfrow = c(1,1))
temp_df <- data.frame(fitted = model7$fitted.values, residual = model7$residuals)
...
```{r}
coef(best_subset_model, 5)
model5 <- lm(MedPrice ~ CrimeRateCat + AvgRoom +  I(1/LStat), data=train)
model7 <- lm(MedPrice ~ CrimeRateCat + AvgRoom + PTRatio + I(1/LStat), data=train)
summary(model5)
# ===== Residuals + QQ Plots =====
m <- model5
...
# Predict MedPrice using model 5 and model 7 on test data
...
# Mean Squared Prediction Error
mspe_model5 <- mean((test$MedPrice - pred_model5)^2)
mspe_model7 <- mean((test$MedPrice - pred_model7)^2)
...
#Do Anova to check if the complex model is significant
anova(model5, model7)

```
Findings:
Model 7 is still the best one as it has smaller MSPE and p value is also better in model 7 i.e p<0.05. p values and anova test between two models tells us that having the extra predictor is producing a better model.
...
#Add RSS, Adjusted R², Cp, and BIC, MSE, etc to the same data frame
mse_values <- best_subset_summary_clean$rss / (nrow(train_clean) - 1:nrow(best_subset_summary_clean$which))
mse_values <- round(mse_values, 2)
